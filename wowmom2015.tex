...
Note that we also allowed the the standard CSMA/CA MAC layer to perform up
to seven retries for a given data packet, by setting the
\texttt{CSMA\_CONF\_MAX\_MAC\_TRANSMISSIONS}
parameter to 8 in Contiki source code. This was done to put it on par
with S-CoSenS default configuration.
However, the ``retry'' term does not have the same signification under
S-CoSenS---where it specifies the reemission of one instance of a
packet---than on ContikiMAC where it specifies a new duty cycle during
