...
specialized OSes for WSN motes have flourished, each one with its strengths
and weaknesses. What are the most important features an OS can offer when
trying to implement a new protocol into its network stack?\\
A second point is the availability of development tools facilitating
implementation and debugging. However, we'll see that such tools are not
necessarily adequate for correct performance evaluation.\\
Finally, what is the impact of implementation choices on the performance
of the resulting software? To what extent does optimization have an impact
on protocols when their implementations are actually in use?\\
...
\begin{observ}
We definitely needed an OS with true real-time features (which basically
implies pre-emptive multitasking), as well as fine granularity
(the 32~KHz rate offered by \texttt{rtimer} is just fine) for
event handling mechanism. Being able to leverage many instances
...
to be near the maximum effective data rate possible (less than 30~Kbit/s),
and the ``extreme'' scenario to be well beyond channel exhaustion.
...
In that case, it depends less on the software platform (OS) itself than
on the implementation choices for the SPI bus driver.
\end{observ}
...
both by the 802.15.4 standard (SFS, LFS), hardware limits of the transceivers,
and overheads due to the various stacks of software layers (especially MAC
and RDC).
...
\item We showed---with the limits and preventions discussed hereabove---that
      ...
      in duty cycle statistics.

\item We discussed problems related to memory corruption we
      encountered---because of very strong constraints on basic WSN motes
      and the absence of a MPU mechanism---and the counter-measures needed
      to handle these problems---on MPU-less architectures, the need to
      survey constantly stack use and memory corruption, using code
      added at the compilation stage.

\item We examined the influence of optimizations on implementation
      performance---specifically, in the management of SPI bus in an
      optimized or safe manner, which impact the speed of loading packets
